...
There is the structure of the training data:
```{r, echo=TRUE}
str(pml_training)
tail(pml_training$X)
...
From this point we can make next conclusions:
...
Resulting training set has next structre: 
```{r, echo=FALSE}
...
# column X can be deleted as well because its just identifier
# tail(pml_training$X)
# pml_training <- preprocessColumns(pml_training, numonly = F, delCol = c("X", "raw_timestamp_part_1","raw_timestamp_part_2","num_window" ))
# pml_training <- preprocessColumns(pml_training, numonly = F, delCol = c("user_name", "cvtd_timestamp", "new_window" ))


# sapply(pml_training, function(x){str(all(is.na(x)))})
# all(is.na(pml_training$min_yaw_arm))

# pml_factors <- pml_training[, sapply(pml_training, is.factor)]
# str(pml_factors)
# 
# pml_factors <- preprocessColumns(pml_factors, numonly = F, delCol = c("user_name", "cvtd_timestamp", "new_window", "classe" ))
# pml_factors <- data.frame(lapply(pml_factors, function(x){as.numeric(as.character(x))}))
# 
# pml_training <- data.frame(classe = pml_training$classe, pml_training[, !sapply(pml_training, is.factor)], pml_factors)
...
#Model training and error estimate
...
## Use tree prediction to estimate variable importanse
...
So due to feature selection we reduces numer of variables to **r length(pml_training)`**
...
confusionMatrix(predict(fit2, newdata = na.omit(testing)), na.omit(testing)$classe)
```
